    ...
    return (
        ...
        <List data={native.devices} columns={[
            ...
            {title: I18n.t("deviceName"), field: "name", format: (data, row) => 
			    ...
			    const p = await (socket as Connection).sendTo(`${connectionInfo.adapterName}.${connectionInfo.instanceId}`, 'getDeviceName', {ip: native.devices[row].ip});
			    if (p == undefined) return;
			    const msg: {success: boolean, name: string} = p as unknown as {success: boolean, name: string};
			    console.log(msg);
			    if (msg.success) {
