...
export default function ObjectList({
  ...
  const addObjectButton = (
    <Button
      ...
      onClick={() => {
        ...
        while (
          state.native.devices[deviceIndex].objects.find(
            (o) => o.objectName == name,
          ) != undefined
        ) {
          num++;
        ...
        setDevices((devices) =>
          devices[deviceIndex].objects.push({
            objectId: 0,
            objectName: name,
            type: 0,
            subscribe: false,
  ...
  return (
    <>
      ...
      <br />
      <List
        data={state.native.devices[deviceIndex].objects}
        columns={[
          {
            ...
            format: (data, row) => (
              <Select
                ...
                onChange={(e) => {
                  setDevices(
                    (devices) =>
                      (devices[deviceIndex].objects[row].type = e.target.value),
                  );
                }}
              >
            ...
            format: (data, row) => (
              <TextField
                ...
                onChange={(e) =>
                  setDevices(
                    (devices) =>
                      (devices[deviceIndex].objects[row].objectId = Number(
                        e.target.value,
                      )),
                  )
                }
          ...
          {
            title: I18n.t("objectDescription"),
            field: "description",
            format: (data, row) => (
              <span
                style={{
              ...
              >
                <TextField
                  ...
                  onChange={(e) =>
                    setDevices(
                      (devices) =>
                        (devices[deviceIndex].objects[row].description =
                          e.target.value),
                    )
                  }
                />
                <Button
                  ...
                  onClick={async () => {
                    const p = await (socket as Connection).sendTo(
                      ...
                      "getObjectDesc",
                      {
                        ip: state.native.devices[deviceIndex].ip,
                        objType:
                          state.native.devices[deviceIndex].objects[row].type,
                        objId:
                          state.native.devices[deviceIndex].objects[row]
                            .objectId,
                      },
                    );
          ...
          {
            hide: !state.expertMode,
            title: I18n.t("objectProperties"),
            field: "props",
            format: (data, row) => (
              <span
                style={{
              ...
              >
                <TextField
                  value={data.map((i) => (isNaN(i) ? "" : i)).join(",")}
                  onChange={(e) =>
                    setDevices((devices) => {
                      devices[deviceIndex].objects[row].props = e.target.value
                        .replace(/[^0-9,]/g, "")
                        .split(",")
            ...
            format: (data, row) => (
              <Checkbox
                ...
                onChange={(e) =>
                  setDevices(
                    (devices) =>
                      (devices[deviceIndex].objects[row].subscribe =
                        e.target.checked),
                  )
                }
              />
        ...
        onDelete={(index) =>
          setDevices((devices) => devices[deviceIndex].objects.splice(index, 1))
        }
      />
